...
const RoutePlanner = () => {
  const [origin, setOrigin] = useState("Golden 700 Golden Ridge Rd. Golden, CO 80401");
  ...
  return (
    <div className="RoutePlannerContainer">
      ...
      <div className="destinationContainer">
        ...
        />
        <div>Destination Count: {waypointCount}</div>
      </div>
      <Button
        onClick={() => {
      ...
      {isLoaded && !!directionServiceOptions && !response && (
        <DirectionsService
          ...
          callback={(res) => {
            setResponse(res.routes[0].legs.map((leg, index) => formatLeg(leg, index)));
          }}
        />
      )}
